...
let largeShirts = runners.filter((sizeL) => {
...
let ticketPriceTotal = runners.reduce((totalDonations, currentValue) => {
...
// Problem 1
// Create an email list on each participants, First name, Last name, and email.

let emailList = [];
runners.forEach((participant) => {
    emailList.push(`${participant.first_name} ${participant.last_name} ${participant.email}`);
});
...
let companyCaps = runners.map((caps) => {
